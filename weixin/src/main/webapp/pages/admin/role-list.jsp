...
<div class="page-container">
		...
		<div class="text-c">
		 ...
		 <span class="select-box inline">
			<select name="status" class="select">
				...
				<option value="1" <c:if test="${roleFrom.status==1 }">selected="selected"</c:if>  >有效</option>
				<option value="0" <c:if test="${roleFrom.status==0 }">selected="selected"</c:if>>无效</option>
			</select>
			<input type="text" name="name" id="" value="${roleFrom.name }"  placeholder=" 角色名称" style="width:250px" class="input-text">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
	 <span class="l">
		 ...
		 <a class="btn btn-primary radius" data-title="添加角色" data-href="pages/admin/role-add.jsp" onclick="Hui_admin_tab(this)" href="javascript:;">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				...
				<tr class="text-c">
					<td><input type="checkbox" value="${l.rid}" name="ids"></td>
					<td>${l.rid}</td>
					<td class="text-l">
					...
					<td class="f-14 td-manage">
						<c:if test="${l.status==1}">
							<a style="text-decoration:none" onClick="role_stop(this,${l.rid})" href="javascript:;" title="下架">
							<i class="Hui-iconfont">&#xe6de;
							</i>
						</a>
						</c:if>
						<c:if test="${l.status==0}">
							<a style="text-decoration:none" onClick="role_start(this,${l.rid})" href="javascript:;" title="上架">
							<i class="Hui-iconfont">&#xe6dc;
							</i>
						</a>
						</c:if>
						
						 <a style="text-decoration:none" class="ml-5" onClick="role_edit('修改','role/select?rid=${l.rid}','10001')" href="javascript:;" title="修改">
						 	<i class="Hui-iconfont">&#xe6df;</i>
						 </a> 
						 <a style="text-decoration:none" class="ml-5" onClick="role_del(this,${l.rid})" href="javascript:;" title="删除">
						 	<i class="Hui-iconfont">&#xe6e2;</i>
						 </a>
					 </td>
				</tr>
...
/*角色-下架*/
function role_stop(obj,rid){
	layer.confirm('确认要下架吗？',function(index){
		$.ajax({
			type:"post",
			url:"role/updateStatus",
			data:{"status":0,"rid":rid},
			success:function(){
				$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="role_start(this,'+rid+')" href="javascript:;" title="上架"><i class="Hui-iconfont">&#xe6dc;</i></a>');
...
function role_start(obj,rid){
	layer.confirm('确认要发布吗？',function(index){
		$.ajax({
			...
			success:function(){
		$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="role_stop(this,'+rid+')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a>');
